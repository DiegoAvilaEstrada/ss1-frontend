...
import { getIngresosPorPeriodo, getGanancias, getCostosNomina } from "@services/reporte.service";
    ...
    try {
      ...
      switch (tipo) {
        case "ingresos":
          ...
          res = await getIngresosPorPeriodo(fechaInicio, fechaFin);
          break;
        case "ganancias":
          if (!fechaInicio || !fechaFin) {
            AlertError("Debe ingresar fecha de inicio y fin");
          ...
          res = await getGanancias(fechaInicio, fechaFin);
          break;
        case "costos-nomina":
          if (!periodo) {
          ...
          res = await getCostosNomina(periodo);
          break;
      }
        ...
        <Grid item xs={12} md={6}>
          <Paper sx={{ p: 3 }}>
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 2 }}>
              ...
              <Button variant="contained" color="primary" onClick={() => handleGenerarReporte("ingresos")} disabled={loading} startIcon={loading ? <CircularProgress size={20} /> : <Assessment />}>
        ...
        <Grid item xs={12} md={6}>
          <Paper sx={{ p: 3 }}>
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 2 }}>
              <TextField label="Fecha Inicio" type="date" value={fechaInicio} onChange={(e) => setFechaInicio(e.target.value)} fullWidth InputLabelProps={{ shrink: true }} />
              <TextField label="Fecha Fin" type="date" value={fechaFin} onChange={(e) => setFechaFin(e.target.value)} fullWidth InputLabelProps={{ shrink: true }} />
              <Button variant="contained" color="primary" onClick={() => handleGenerarReporte("ganancias")} disabled={loading} startIcon={loading ? <CircularProgress size={20} /> : <Assessment />}>
        ...
        <Grid item xs={12} md={6}>
          <Paper sx={{ p: 3 }}>
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 2 }}>
              ...
              <Button variant="contained" color="primary" onClick={() => handleGenerarReporte("costos-nomina")} disabled={loading} startIcon={loading ? <CircularProgress size={20} /> : <Assessment />}>
